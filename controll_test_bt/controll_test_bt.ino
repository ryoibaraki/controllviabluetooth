...
{ 
  ...
  */
  ...
  if (Serial1.available() > 0) {
    ...
    inputchar = Serial1.read(); //シリアル通信で送信された値を読み取る
  ...
  Serial1.println(tgt_spd);
  for(int i = 0; i < 3; i++){
  Serial1.print(i);
  Serial1.print(": ");
  Serial1.print(" ");
  Serial1.print(omega_d[i]);
  Serial1.print(" ");
  Serial1.print(omega[i]);
  Serial1.print(" ");
  Serial1.println(mv[i]);
  }
  Serial1.println(t_delay);
  Serial1.println();
  
  int tc2 = millis();
  
  //Serial1.println(mv[2]);
  //t_old = millis();
